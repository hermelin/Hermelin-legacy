...
ext = {
  ...
  init: function init() {
    ...
    var ui_template_form_status_indicators = ui.Template.form_status_indicators;
  ...
  init_exts: function init_exts() {
    ...
    for (var key in ext) {
      ...
      if (65 <= key.charCodeAt(0) && key.charCodeAt(0) <= 90 && typeof ext[key].enable == 'function' && typeof ext[key].disable == 'function') {
  ...
  enable_ext: function enable_ext(id) {
    try {
  ...
  disable_ext: function disable_ext(id) {
    try {
  ...
  config_ext: function config_ext(id) {
    try {
  ...
  load_builtin_exts: function load_builtin_exts(callback) {
    var path_arr = [];
  ...
  load_exts: function load_exts(type, exts, callback) {
    var procs = [];
  ...
  register_listener: function register_listener(type, callback) {
  ...
  unregister_listener: function unregister_listener(type, callback) {
    if (ext.listeners.hasOwnProperty(type)) {
  ...
  add_exts_menuitem: function add_exts_menuitem(id, icon, label, callback) {
  ...
  remove_exts_menuitem: function remove_exts_menuitem(id) {
    var a = $('#exts_menu .' + id)
    a.unbind('click');
    var li = a.parent();
    li.remove();
  ...
  add_context_menuitem: function add_context_menuitem(id, label, select_only, callback) {
  ...
  remove_context_menuitem: function remove_context_menuitem(id) {
  ...
  add_tweet_more_menuitem: function add_tweet_more_menuitem(id, label, type, callback) {
  ...
  remove_tweet_more_menuitem: function add_tweet_more_menuitem(id) {
    var a = $('#ext_tweet_' + id);
    a.unbind('click');
    var li = a.parent();
    li.remove();
...
ext.Preferences = function (prefs_name) {
  ...
  function get(key, callback) {
    var _this = this;
    ext.prefs.transaction(function (tx) {
      tx.executeSql('SELECT name, val FROM "' + _this.name + '" WHERE name=?', [key], function (tx, rs) {
        if (callback) {
          var val = null;
          if (rs.rows.length != 0) {
  ...
  function set(key, val, callback) {
    ...
    var _this = this;
    ext.prefs.transaction(function (tx) {
      tx.executeSql('INSERT or REPLACE INTO "' + _this.name + '" VALUES (?, ?)', [key, val], function (tx, rs) {
        if (callback) {
          callback(key, val);
        }
      }, function (tx, error) {
