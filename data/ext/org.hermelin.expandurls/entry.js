// org.hermelin.expandurls - hermelin extension to expand shortened urls (refs #368)
...
ext.ExpandUrls = {
  ...
  longurl_service: {
    name: 'longurlplease',
    service_url: 'http://api.longurl.org/v2/expand?user-agent=Hermelin&format=json&url=',
    ...
    // I do not understand; it is probably some hermelin magic :-)
  ...
  on_add_tweets: function on_add_tweets(tweets, view) {
    ...
    for (var i = 0, l = tweets.length; i < l; ++i) {
      ...
      while (match != null) {
        var req_url = ext.ExpandUrls.longurl_service.service_url + encodeURIComponent(match[0]);
        ...
        $.ajax({
          ...
          context: {
            short_url: match[0]
          } // definition of 'this' in callback
          ,
          success: function (results) {
            var long_url = results["long-url"];

            // Log what has been found
            hermelin_log('ExpandUrls', this.short_url + ' : ' + long_url);
  ...
  enable: function enable() {
    ext.register_listener(ext.ADD_TWEETS_LISTENER_AFTER, ext.ExpandUrls.on_add_tweets);
  },

  disable: function disable() {
    ext.unregister_listener(ext.ADD_TWEETS_LISTENER_AFTER, ext.ExpandUrls.on_add_tweets);
  }
