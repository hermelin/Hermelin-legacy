...
ext.HototStat = {
  ...
  header_html: '<ul id="ext_hototstat_btns" class="radio_group">\
  ...
  body_html: '<div id="ext_hototstat_home" class="dialog_block" style="text-align:center">\
...
<div id="ext_hototstat_relation" class="dialog_block" style="display:none">\
  ...
  fetch_user_tweets: function fetch_user_tweets(screen_name, callback) {
    ...
    var fetch_proc = function () {
      frame_win.progress_set_label('Loading: ' + ext.HototStat.user_stat_info.summary.total_count);
  ...
  fetch_home_tweets: function fetch_home_tweets(callback) {
    ...
    frame_win.start();
    var fetch_proc = function () {
      frame_win.progress_set_label('Loading: ' + ext.HototStat.home_stat_info.summary.total_count + '/1000');
  ...
  on_btn_update_clicked: function on_btn_update_clicked(event) {
    if (ext.HototStat.current == '#ext_hototstat_user') {
      ...
      ext.HototStat.fetch_user_tweets(screen_name, function () {
        ext.HototStat.update_stat_frame('user');
      });
    } else if (ext.HototStat.current == '#ext_hototstat_home') {
      ext.HototStat.reset();
      ext.HototStat.fetch_home_tweets(function () {
      ...
      ext.HototStat.get_relationship(screen_name1, screen_name2, function (relation) {
        switch (relation) {
        case 0:
  ...
  enable: function enable() {
    ext.add_exts_menuitem('ext_btn_hotot_stat_image', ext.HototStat.id + '/16.png', 'View Tweet Stat ...', ext.HototStat.on_ext_btn_clicked);
    // create stat dialog
    var title = 'Tweet Stat ...'
    ext.HototStat.stat_dialog = widget.DialogManager.build_dialog('#ext_imagestat_dialog', title, ext.HototStat.header_html, ext.HototStat.body_html, [{
        id: '#ext_hototstat_close_btn',
        label: 'Close',
        click: ext.HototStat.on_btn_close_clicked
               }, {
        id: '#ext_hototstat_update_btn',
        label: 'Update',
        click: ext.HototStat.on_btn_update_clicked
              }]);
    ext.HototStat.stat_dialog.set_styles('header', {
      'padding': '10px'
    })
    ext.HototStat.stat_dialog.set_styles('body', {
      'padding': '0px',
