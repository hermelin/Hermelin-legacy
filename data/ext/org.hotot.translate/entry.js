...
ext.HototTranslate = {
  ...
  do_translate_selection: function do_translate_selection(dst_lang) {
    ...
    ext.HototTranslate.do_translate(dst_lang, text, function (result) {
      var content = '';
      if (result.responseStatus == 200) {
        content = '<strong>Source</strong>:<p style="' + style + '">' + text + '</p><strong>' + ext.HototTranslate.languages[dst_lang] + '</strong>:<p style="' + style + '">' + result.responseData.translatedText + '</p>';
      } else {
  ...
  do_translate_tweet: function do_translate_tweet(li_id, dst_lang) {
    ...
    var style = 'background:transparent url(' + 'ext/' + ext.HototTranslate.id + '/ic16_translate.png' + ') no-repeat;padding-left:20px;';
    ext.HototTranslate.do_translate(dst_lang, text.attr('alt') || text.text(), function (result) {
      var content = '';
      if (result.responseStatus == 200) {
        content = '<strong style="' + style + '">' + result.lang + '&rarr;' + ext.HototTranslate.languages[dst_lang] + '</strong>: ' + result.responseData.translatedText;
      } else {
  ...
  create_option_dialog: function create_option_dialog() {
    ...
    var body = '<p>\
        ...
        <center><select id="ext_hotot_translate_dst_language" title="Choose a destination language." class="dark"></center>' + options_arr.join() +
      '</select></p>';

    ext.HototTranslate.option_dialog = widget.DialogManager.build_dialog(
      '#ext_hotot_translate_opt_dialog', title, '', body, [{
        id: '#ext_btn_hotot_translate_save',
        label: 'Save',
        click: ext.HototTranslate.on_btn_save_prefs_clicked
      }]);
    ext.HototTranslate.option_dialog.set_styles('header', {
      'padding': '0',
      'display': 'none',
      'height': '0'
    });
  ...
  enable: function enable() {
    ext.add_context_menuitem('ext_btn_hotot_translate', 'Translate Selection.', true, ext.HototTranslate.on_centext_mitem_clicked);

    ext.add_tweet_more_menuitem('ext_btn_hotot_translate', 'Translate', true, ext.HototTranslate.on_tweet_more_mitem_clicked);
