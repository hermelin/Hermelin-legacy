...
ext.HototShortUrl = {
  ...
  create_option_dialog: function create_option_dialog() {
    ...
    ext.HototShortUrl.option_dialog = widget.DialogManager.build_dialog(
      '#ext_hotot_short_url_opt_dialog', title, '', body, [{
        id: '#ext_btn_hotot_short_url_save',
        label: 'Save',
        click: ext.HototShortUrl.on_btn_save_prefs_clicked
      }]);
    ext.HototShortUrl.option_dialog.set_styles('header', {
      'display': 'none',
      'height': '0'
    });
    ...
    $('#ext_hotot_short_url_service').change(function () {
      ...
      service.url ? $('#ext_hotot_short_url_other').hide() : $('#ext_hotot_short_url_other').show();
      service.params.login ? $('#ext_hotot_short_url_login').show() : $('#ext_hotot_short_url_login').hide();
      service.params.yourlsbase ? $('#ext_hotot_short_url_yourlsbase').show() : $('#ext_hotot_short_url_yourlsbase').hide();
      service.params.api_key ? $('#ext_hotot_short_url_api_key').show() : $('#ext_hotot_short_url_api_key').hide();
      (service.params.api_key && service.params.api_key.help) ? $('#ext_hotot_short_url_api_key a').attr(
        'href', service.params.api_key.help).attr(
        'title', 'Where is my ' + service.name + ' API Key?').css(
        'color', '#fff').show() : $('#ext_hotot_short_url_api_key a').hide();
      ext.HototShortUrl.prefs.get('other', function (key, val) {
