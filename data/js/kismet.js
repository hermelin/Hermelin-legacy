 ...
 */
  ...
  save: function save() {
    if (typeof conf != 'undefined') {
      conf.get_current_profile().preferences.kismet_colored_user_map = kismet.colored_user_map;
  ...
  update_rule: function update_rule(rule) {
    var notfound = true;
    for (var i = 0; i < kismet.rules.length; i += 1) {
  ...
  remove_rule: function remove_rule(name) {
    for (var i = 0; i < kismet.rules.length; i += 1) {
  ...
  update_mute_list: function update_mute_list(field, value) {
  ...
  set_user_color: function set_user_color(screen_name, color) {
  ...
  get_user_color: function get_user_color(screen_name) {
  ...
  eval_bool_exp: function eval_bool_exp(exp, incoming) {
    if (!exp) return false;
  ...
  eval_cond: function eval_cond(cond, incoming) {
    if (cond.length == 1) {
  ...
  do_action: function do_action(rule, incoming) {
    var ret = true;
  ...
  do_drop: function do_drop(rule, act, incoming) {
  ...
  do_archive: function do_archive(rule, act, incoming) {
    ...
    if (!ui.Main.views.hasOwnProperty('kismet_' + formal_name)) {
      ui.Slider.add('kismet_' + formal_name, {
        title: 'Kismet # ' + rule.name,
        icon: 'image/ic_archive.png'
      }, {
        'type': 'tweet',
        'title': 'Kismet # ' + rule.name,
        'load': null,
  ...
  do_reply: function do_reply(rule, act, incoming) {
  ...
  do_quote: function do_quote(rule, act, incoming) {
  ...
  do_mask: function do_mask(rule, act, incoming) {
    incoming.text = kismet.MASK_TEXT;
    // console.log('[ACT]', 'Mask the incoming!');
  },

  filter_proc: function filter_proc(single) {
    var ret = true;
  ...
  filter: function filter(incoming) {
    return incoming.filter(kismet.filter_proc);
  },

  get_holder_value: function get_holder_value(name, tweet) {
    ...
    case '$HASHTAGS':
      if (real_tweet.entities && real_tweet.entities.hashtags)
        return real_tweet.entities.hashtags.map(function (t) {
          return t.text
        });
      else
        return [];
    ...
    case '$MENTIONS':
      if (real_tweet.entities && real_tweet.entities.user_mentions)
        return real_tweet.entities.user_mentions.map(function (t) {
          return t.screen_name.toLowerCase()
        });
      else
        return [];
      break;
    case '$LINKS':
      if (real_tweet.entities && real_tweet.entities.urls)
        return real_tweet.entities.urls.map(function (t) {
          return t.expanded_url
        });
      else
        return [];
      break;
  ...
  process_action: function process_action(tokens, pos) {
    switch (tokens[pos][1]) {
    case 'drop':
  ...
  process_has: function process_has(tokens, pos) {
    switch (tokens[pos][1]) {
    case 'map':
    case 'geo':
  ...
  process_field: function process_field(tokens, pos) {
  ...
  recognize_string: function recognize_string(str, pos) {
    var ch = str[pos];
    var last = ch;
  ...
  recognize_re: function recognize_re(str, pos) {
    var ch = str[pos];
    var last = ch;
    while (pos < str.length) {
      last = ch;
  ...
  recognize_keyword: function recognize_keyword(str, pos) {
    var ch = str[pos];
    var last = ch;
    while (pos < str.length) {
      last = ch;
  ...
  read_tokens: function read_tokens(str) {
    // @TODO 
    var token_list = [];
    var pos = 0,
      end_pos = 0;
  ...
  compile: function compile(str) {
    ...
    kismet.rule_string = kismet.action_string_array.join(' and ') + ' if it ' + kismet.cond_string_array.join(' and ');
