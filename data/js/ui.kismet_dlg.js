...
ui.KismetDlg = {
  init: function init() {
    ...
    $('#kismet_guide_next_btn').click(function () {
      ...
      if (marked.length === 1 && columns.length == 0 && (actions.length == 0 || actions.length === 1 && actions.val() === 'drop')) { // simple, use mute list
        var field = 'word';
  ...
  switch_category: function switch_category(name) {
    $('#kismet_dialog .kismet_page').hide();
    $('#kismet_dialog .' + name).show();
  },

  show_edit_dialog: function show_edit_dialog(default_value) {
    if (default_value) {
  ...
  load_guide: function load_guide(tweet) {
  ...
  reload_mute_list: function reload_mute_list() {
  ...
  reload_rules: function reload_rules() {
