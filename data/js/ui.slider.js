...
ui.Slider = {
  ...
  init: function init() {
    ...
    $('#slider_menu a').click(function () {
      ...
      case 'list':
        ret = widget.DialogManager.prompt('Input a List name:',
          'form: @screenname/slug', function (ret) {
          ret = $.trim(ret);
          if (ret.length !== 0) {
            if (ret[0] == '@') ret = ret.substring(1);
    ...
    $('#slider_menu').mouseleave(function (event) {
      ui.Slider.closeSliderMenu();
    });
    ...
    $('#view_setting_menu').mouseleave(function (event) {
  ...
  resume_state: function resume_state() {
  ...
  add: function add(name, indicator_opts, view_opts) {
  ...
  add_indicator: function add_indicator(name, opts) {
  ...
  add_view: function add_view(name, opts) {
    // create & add view
  ...
  remove_view: function remove_view(name) {
  ...
  remove_indicator: function remove_indicator(name) {
  ...
  slide_to_prev: function slide_to_prev(no_loop) {
    var prev_id = '';
  ...
  slide_to_next: function slide_to_next(no_loop) {
    var next_id = '';
    var idx = ui.Slider.get_page_pos(ui.Slider.current);
  ...
  set_icon: function set_icon(name, icon, style) {
  ...
  set_icon_alt: function set_icon_alt(name, icon) {
    var btn = $('#indication .indicator_btn[href="#' + name + '"]');
  ...
  set_unread: function set_unread(name) {
    var btn, btns = document.getElementById('indication').getElementsByClassName('indicator_btn');
    for (var i = 0; i < btns.length; i++) {
      if (btns[i].parentNode.getAttribute('name') === name) {
        btn = btns[i];
        break;
      }
    }
  ...
  mark_read: function mark_read(name) {
    var btn = $('#indication .indicator_btn[href="#' + name + '"]');
    btn.removeClass('unread');
  },

  openSliderMenu: function openSliderMenu() {
    $('#indicator_add_btn').addClass('hlight');
    $('#slider_menu').css({
      'left': ($('#indicator_add_btn').offset().left) + 'px'
    }).show();
  ...
  closeSliderMenu: function closeSliderMenu() {
  ...
  indicator_btn_drag_enter: function indicator_btn_drag_enter(ev) {
    $(this).addClass('over');
  },

  indicator_btn_drag_leave: function indicator_btn_drag_leave(ev) {
    $(this).removeClass('over');
  },

  indicator_btn_drag_end: function indicator_btn_drag_end(ev) {
    $('#indicator_btns > div').removeClass('over');
    $(this).css('opacity', '1');
    return false;
  },

  indicator_btn_drag_over: function indicator_btn_drag_over(ev) {
  ...
  indicator_btn_drop: function indicator_btn_drop(ev) {
    if (ev.originalEvent.stopPropagation) {
      ev.originalEvent.stopPropagation();
    }
    var src_name = ev.originalEvent.dataTransfer.getData('text/html');
    var my_name = $(this).attr('name');
    if (my_name == 'search' || ui.Slider.tweet_blocks.indexOf(my_name) == -1 || ui.Slider.tweet_blocks.indexOf(src_name) == -1) {
      return false;
    }
    if (my_name != src_name) {
  ...
  swap_indicator: function swap_indicator(src_name, dst_name) {
  ...
  swap_view: function swap_view(src_name, dst_name) {
  ...
  bind_common_settings: function bind_common_settings(view) {
  ...
  addDefaultView: function addDefaultView(name, opts) {
