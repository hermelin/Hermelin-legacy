...
(function () {
  var getActiveWindow, install, onExtRequest, onTabCreated, onTabRemoved, onTabUpdated, root, shareLink, sharePage, shareSelection, shareWithHermelin, showHermelinTab, tabChangedHandler, uninstall, hermelinIndexURL;

  hermelinIndexURL = chrome.extension.getURL("index.html");

  tabChangedHandler = function (tab) {
    if (tab.url.indexOf(hermelinIndexURL) !== -1) {
  ...
  showHermelinTab = function () {
    ...
    if (root._hermelinTab) {
      ...
      chrome.tabs.get(root._hermelinTab.id, function (c) {
        root._hermelinTab = c;
  ...
  shareWithHermelin = function (str) {
    var _doShare;
    _doShare = function () {
      var win, _testProc;
      win = getActiveWindow();
    ...
    if (root._hermelinTab && root._hermelinTab.id) {
      showHermelinTab();
      _doShare();
    } else {
      chrome.tabs.create({
  ...
  install = function () {
    ...
    if (root._menuItemSharePageId === null) {
      root._menuItemSharePageId = chrome.contextMenus.create({
        "title": "Share Page with Hermelin",
        "contexts": ["page"],
        "onclick": sharePage
      });
    }
    if (root._menuItemShareSelId === null) {
      root._menuItemShareSelId = chrome.contextMenus.create({
        "title": "Share Selection with Hermelin",
        "contexts": ["selection"],
    ...
    if (root._menuItemShareLinkId === null) {
      root._menuItemShareLinkId = chrome.contextMenus.create({
        "title": "Share Link with Hermelin",
        "contexts": ["link"],
        "onclick": shareLink
      });
    }
