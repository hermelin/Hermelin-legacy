...
ui.SearchView = {
  ...
  load_tweet: function load_tweet(view, success, fail) {
    /* sample response
    ...
    */
    ...
    if (view.type != 'saved_search') {
      globals.twitterClient.show_user(view.query, function (user) {
  ...
  loadmore_tweet: function loadmore_tweet(view, success, fail) {
    if (!view.page) view.page = 1;
    globals.twitterClient.search(view.query, view.page + 1, null, view.max_id, success);
  },

  load_people: function load_people(view, success, fail) {
    if ($.trim(view.query).length == 0) {
      success([]);
      return;
    }
  ...
  loadmore_people: function loadmore_people(view, success, fail) {
    globals.twitterClient.search_user(view.query, view.page, success, fail);
  },

  load_tweet_success: function load_tweet_success(view, json) {
    var tweets = [];
  ...
  load_tweet_fail: function load_tweet_fail(view, json) {
    if (json && json.ignore) {
      view._header.find('.search_no_result_hint').hide();
    }
  },

  loadmore_tweet_success: function loadmore_tweet_success(view, json) {
    if (view.page) view.page += 1;
    var tweets = [];
    if (json.constructor == Object && (json.results != undefined || json.statuses != undefined)) {
      tweets = json.results || json.statuses;
  ...
  load_people_success: function load_people_success(view, json) {
    ui.Slider.set_unread(view.name);
    return ui.Main.add_people(view, json);
  },

  loadmore_people_success: function loadmore_people_success(view, json) {
    ui.Slider.set_unread(view.name);
    return ui.Main.add_people(view, json);
  },

  do_search: function do_search(view, query) {
  ...
  clear: function clear(view, noTrend) {
    view._header.find('.search_people_result').hide();
  ...
  get_trending_topics_local: function get_trending_topics_local(view, success, fail) {
  ...
  get_trending_topics_worldwide: function get_trending_topics_worldwide(view, success, fail) {
    globals.twitterClient.get_trending_topics(1, success);
    return 1; // There are always trend topics
  },

  get_trending_topics_success: function get_trending_topics_success(self, json) {
