...
function update_tweet_block_width() {
  ...
  if (ui.Slider.column_num != 0) {
    ...
    for (var i = 0; i < viewtitles.length; i++) {
      ...
      if (i === (ui.Slider.column_num - 1)) {
        viewtitles[i].style.width = (lastColumnWidth - 1) + 'px';
      } else {
        viewtitles[i].style.width = (globals.tweet_block_width - 1) + 'px';
      }
    }
...
function hotot_log(label, content) {
  if (conf.settings.use_verbose_mode) {
    if (util.is_native_platform()) {
      hotot_action('action/log/' + encodeURIComponent(label) + '/' + encodeURIComponent(content));
  ...
  if (util.is_native_platform() && globals.load_flags == 0) {
    ...
    if (util.is_native_platform()) {
      procs.push(function () {
        ...
        hotot_action('system/load_settings/' + encodeURIComponent(JSON.stringify(conf.settings)))
