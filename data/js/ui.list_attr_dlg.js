...
ui.ListAttrDlg = {
  ...
  init: function init() {
    ...
    $('#list_attr_slug').keyup(function (event) {
      ui.ListAttrDlg.limit_test(this, 20);
      return false;
    });
    $('#list_attr_desc').keyup(function (event) {
  ...
  update: function update() {
    ...
    if (ui.ListAttrDlg.create_new) {
      ...
      globals.twitterClient.create_list(slug, description, mode, function (result) {
        toast.set("Create List successfully!").show();
        globals.list_attr_dialog.close();
      });
    } else {
      toast.set("Update List ...").show();
      globals.twitterClient.update_list(ui.ListAttrDlg.owner, slug, description, mode, function (result) {
  ...
  load: function load(screen_name, slug) {
    if (slug == '') {
    ...
    globals.twitterClient.show_list(screen_name, slug, function (list_obj) {
