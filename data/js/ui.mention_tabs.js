...
ui.MentionTabs = {
  ...
  get_non_reply_tweets: function get_non_reply_tweets(tweets) {
    return $($.grep(tweets, function (obj, i) {
      return $(obj).attr('reply_name') != globals.myself.screen_name;
    }));
  },

  get_reply_tweets: function get_reply_tweets(tweets) {
    return $($.grep(tweets, function (obj, i) {
      return $(obj).attr('reply_name') == globals.myself.screen_name;
    }));
  },
  ...
  apply_filter: function apply_filter() {
    ...
    case '#reply':
      ui.MentionTabs.get_non_reply_tweets(
        $('#mentions_tweet_block .card:visible')).hide();
      ui.MentionTabs.get_reply_tweets(
        $('#mentions_tweet_block .card:hidden')).show();
      break;
    default:
      break;
    }
