...
try:
    import glib
    glib.set_application_name(_("Hermelin"))
except:
    pass

HERMELIN_DBUS_PATH = '/org/hermelin/service'
HERMELIN_DBUS_NAME = 'org.hermelin.service'

class HermelinDbusService(dbus.service.Object):
    def __init__(self, app):
        bus_name = dbus.service.BusName(HERMELIN_DBUS_NAME, bus=dbus.SessionBus())
        dbus.service.Object.__init__(self, bus_name, HERMELIN_DBUS_PATH)
        self.app = app
        s = []

    @dbus.service.method(dbus_interface=HERMELIN_DBUS_NAME, in_signature="", out_signature="i")
    def unread(self):
        return self.app.state['unread_count']

    @dbus.service.signal(dbus_interface=HERMELIN_DBUS_NAME)
    def incoming(self, group, tweets):
        pass

    @dbus.service.method(dbus_interface=HERMELIN_DBUS_NAME, in_signature="s", out_signature="")
    def update_status(self, text):
        self.app.update_status(text)

    @dbus.service.method(dbus_interface=HERMELIN_DBUS_NAME, in_signature="", out_signature="")
    def show(self):
...
class Hermelin:
    def __init__(self):
        ...
        self.dbus_service = HermelinDbusService(self)
    ...
    def unread_alert(self, subtype, sender, body="", count=0):
        ...
        if not HAS_INDICATOR:
            self.trayicon.set_tooltip("Hermelin: %d unread tweets/messages." % count if count > 0 else _("Hermelin: Click to Active."))
    ...
    def init_hotkey(self):
        try:
            import keybinder
            keybinder.bind(
                  config.settings['shortcut_summon_hermelin']
    ...
    def create_trayicon(self):
        ...
        self.trayicon.set_tooltip(_("Hermelin: Click to Active."))
