...
ext.AppMask = {
  ...
  open_option_dialog: function open_option_dialog() {
    ...
    var body = '<style>' +
      '#ext_hermelin_appmask_opt_dialog .dialog_body>ol{list-style-type:decimal;margin-left:25px}' +
      '#ext_hermelin_appmask_opt_dialog .dialog_body li{margin:10px 0}' +
      '#ext_hermelin_appmask_opt_dialog .dialog_body table{margin:0 10px}' +
      '#ext_hermelin_appmask_opt_dialog .dialog_body td{border:none;padding:0px}' +
      '#ext_hermelin_appmask_opt_dialog .dialog_body input{width:200px}' +
      '</style>' +
      '<ol>' +
      '<li>' +
      ...
      '<tr>' +
      '<td>Consumer Key:</td>' +
      '<td><input type="text" id="ext_hermelin_appmask_consumer_key"/></td>' +
      '</tr>' +
      '<tr>' +
      '<td>Consumer Secret:</td>' +
      '<td><input type="text" id="ext_hermelin_appmask_consumer_secret"/></td>' +
      '</tr>' +
      '</tbody>' +
      '</table>' +
      '</li>' +
    ...
    ext.AppMask.option_dialog = widget.DialogManager.build_dialog(
      '#ext_hermelin_appmask_opt_dialog', title, '', body, [
      {
        id: '#ext_btn_appmask_reset',
        label: 'Reset',
        click: ext.AppMask.on_btn_reset_clicked
      },
      {
        id: '#ext_btn_appmask_save',
        label: 'Save',
        click: ext.AppMask.on_btn_save_prefs_clicked
      }
    ]);
    ext.AppMask.option_dialog.set_styles('header', {
      'display': 'none',
      'height': '0'
    });
    ...
    for (var key in prefs) {
      $('#ext_hermelin_appmask_' + key).val(prefs[key]);
    }

    ext.AppMask.option_dialog.open();
  },

  on_btn_reset_clicked: function on_btn_reset_clicked() {
  ...
  on_btn_save_prefs_clicked: function on_btn_save_prefs_clicked() {
    ...
    for (var key in prefs) {
      var value = $('#ext_hermelin_appmask_' + key).val();
      if (value) {
        prefs[key] = value;
      } else {
        toast.set("Please fill the " + key.replace("_", " ") + " field").show();
        $('#ext_hermelin_appmask_' + key).focus();
        return;
      }
    }
