...
conf = {
  ...
  vars: {
    ...
    'version': 'alpha 0.1',
    'codename': 'Sunrise',
    'consumer_key': 'crdJOo1I9t59sS7bQ7dXw',
    'consumer_secret': 'XNwwRxOvPMskpGJKc0HeXscGsOLhAVghNhyNq38tFk',
  ...
  get_default_prefs: function get_default_prefs(protocol) {
    if (protocol == 'twitter') {
  ...
  save_settings: function save_settings(callback) {
  ...
  load_settings: function load_settings(callback) {
    db.load_option('settings', function (settings) {
  ...
  save_prefs: function save_prefs(name, callback) {
    var profile = {};
  ...
  load_prefs: function load_prefs(name, callback) {
    db.get_profile(name, function (profile) {
  ...
  apply_settings: function apply_settings() {
    $('.version').text(conf.vars.version + ' (' + conf.vars.codename + ')');
  ...
  apply_prefs: function apply_prefs(name, full) {
  ...
  load_token: function load_token(name) {
    return conf.profiles[name].preferences.access_token;
  },

  save_token: function save_token(name, token) {
    conf.profiles[name].preferences.access_token = token;
    conf.save_prefs(name);
  },

  clear_token: function clear_token(name) {
    conf.profiles[name].preferences.access_token = '';
    conf.save_prefs(name);
  },

  normalize_prefs: function normalize_prefs(protocol, prefs) {
  ...
  normalize_settings: function normalize_settings(settings) {
  ...
  get_default_font_settings: function get_default_font_settings() {
    var platform = navigator.platform;
    if (conf.OS_WINDOWS) {
