...
ui.Main = {
  ...
  init: function init() {
    ...
    $('#tweet_set_color_btn').click(function (ev) {
      var li = $(ui.Main.active_tweet_id);
  ...
  load_home: function load_home(self, success, fail) {
    globals.twitterClient.get_home_timeline(
      self.since_id, null, conf.vars.items_per_request,
      success);
  },

  loadmore_home: function loadmore_home(self, success, fail) {
  ...
  load_mentions: function load_mentions(self, success, fail) {
    globals.twitterClient.get_mentions(
      self.since_id, null, conf.vars.items_per_request,
      success);
  },

  loadmore_mentions: function loadmore_mentions(self, success, fail) {
    globals.twitterClient.get_mentions(
      null, self.max_id, conf.vars.items_per_request,
      success);
  },

  load_messages: function load_messages(self, success, fail) {
    var since_id = self.since_id;
    globals.twitterClient.get_direct_messages(
      since_id, null, conf.vars.items_per_request, success);
    globals.twitterClient.get_sent_direct_messages(
      since_id, null, conf.vars.items_per_request, success);
  },

  loadmore_messages: function loadmore_messages(self, success, fail) {
    globals.twitterClient.get_direct_messages(
      null, self.max_id, conf.vars.items_per_request,
      success);
    globals.twitterClient.get_sent_direct_messages(
  ...
  load_tweet_success: function load_tweet_success(self, json) {
  ...
  load_people_success: function load_people_success(self, json) {
  ...
  load_list_success: function load_list_success(self, json) {
  ...
  load_listed_list_success: function load_listed_list_success(self, json) {
  ...
  loadmore_tweet_success: function loadmore_tweet_success(self, json) {
  ...
  loadmore_people_success: function loadmore_people_success(self, json) {
    var ret = ui.Main.add_people(self, json.users);
    if (0 < self.incoming_num) {
      ui.Slider.set_unread(self.name);
    }
  ...
  loadmore_listed_list_success: function loadmore_listed_list_success(self, json) {
    var ret = ui.Main.add_people(self, json.lists);
    if (0 < self.incoming_num) {
      ui.Slider.set_unread(self.name);
    }
  ...
  add_people: function add_people(self, users) {
  ...
  add_tweets: function add_tweets(self, json_obj, reversion, ignore_kismet) {
    ...
    if (ui.Main.use_preload_conversation && self.hasOwnProperty('_me')) {
      for (var i = 0; i < json_obj.length; i += 1) {
        if (json_obj[i].in_reply_to_status_id_str == null && json_obj[i].in_reply_to_status_id == null) {
          continue;
        }
        var dom_id = self.name + '-' + json_obj[i].id_str;
  ...
  sort: function sort(json_obj, reversion) {
  ...
  insert_isolated_tweet: function insert_isolated_tweet(self, tweet, reversion) {
  ...
  get_next_tweet_dom: function get_next_tweet_dom(view, current, reversion) {
    ...
    var next_one = null;
    if (current == null) {
      next_one = reversion ? view._body.find('.card:last') : view._body.find('.card:first');
    } else {
  ...
  bind_tweet_action: function bind_tweet_action(id) {
    ...
    $(id).find('a[direct_url]').click(function () {
      ...
      if (typeof (direct_url) != 'undefined') {
        var video = $(this).attr('video') === "true";
        ui.Previewer.reload(direct_url, video);
        ui.Previewer.open();
        return false;
      }
      return false;
    ...
    $(id).find('.btn_tweet_thread:first').click(function (event) {
      ui.Main.on_expander_click(this, event);
    });

    $(id).find('.btn_tweet_thread_more:first').click(function (event) {
    ...
    $(id).find('.tweet_retweet_btn').click(function (ev) {
    ...
    $(id).find('.tweet_fav_btn').click(function (event) {
      ui.Main.on_fav_click(this, ui.Main.active_tweet_id, event);
      return false;
    });

    // type: message
    $(id).find('.tweet_dm_reply_btn').click(function (event) {
      ui.Main.on_dm_click(this, ui.Main.active_tweet_id, event);
      return false;
    });
    $(id).find('.tweet_dm_delete_btn').click(function (event) {
      ui.Main.on_dm_delete_click(this, ui.Main.active_tweet_id, event);
      return false;
    });

    // type: people
    $(id).find('.follow_btn').click(function (event) {
      ui.Main.on_follow_btn_click(this, ui.Main.active_tweet_id, event);
    });
    $(id).find('.unfollow_btn').click(function (event) {
  ...
  bind_tweet_text_action: function bind_tweet_text_action(id) {
    ...
    $(id).find('.list_href').click(function (event) {
  ...
  unbind_tweet_action: function unbind_tweet_action(li_id) {
    $(li_id + ' a').unbind();
    $(li_id).unbind();
  },

  on_reply_click: function on_reply_click(btn, li_id, event) {
    var li = $(li_id);
    var id = (li.attr('retweet_id') == '' || li.attr('retweet_id') == undefined) ? li.attr('tweet_id') : li.attr('retweet_id');
    var screen_name = li.attr('screen_name');
  ...
  on_rt_click: function on_rt_click(btn, li_id, event) {
    var li = $(li_id);
    var screen_name = li.attr('screen_name');
    var _text = $(li.find('.text')[0]);
    var text = _text.attr('alt') || _text.text();

    ui.StatusBox.set_status_text(" RT @" + screen_name + ': ' + text + ' ');
    ui.StatusBox.open(function () {
  ...
  on_reply_all_click: function on_reply_all_click(btn, li_id, event) {
    var li = $(li_id);
    var id = (li.attr('retweet_id') == '' || li.attr('retweet_id') == undefined) ? li.attr('tweet_id') : li.attr('retweet_id');
    var screen_name = li.attr('screen_name');
    var text = $(li.find('.text')[0]).text();
  ...
  on_dm_click: function on_dm_click(btn, li_id, event) {
    var li = $(li_id);
    var screen_name = (li.attr('screen_name') == '' || li.attr('screen_name') == undefined) ? li.attr('sender_screen_name') : li.attr('screen_name');
  ...
  on_del_click: function on_del_click(btn, li_id, event) {
    var li = $(li_id);
    var id = (li.attr('retweet_id') == '' || li.attr('retweet_id') == undefined) ? li.attr('tweet_id') : li.attr('retweet_id');
    ...
    if (li.attr('type') === 'message') {
      ...
      globals.twitterClient.destroy_status(id, function (result) {
        ui.Main.unbind_tweet_action(li_id);
        li.remove();
        toast.set(_('destroy_successfully')).show();
      });
  ...
  on_follow_btn_click: function on_follow_btn_click(btn, li_id, event) {
    var li = $(li_id);
    if (li.attr('type') == 'people') {
      var screen_name = li.attr('screen_name');
      toast.set(_('follow_at') + screen_name + ' ' + _('dots')).show();
      globals.twitterClient.create_friendships(screen_name, function () {
        toast.set(
      ...
      globals.twitterClient.create_list_subscriber(screen_name, slug, function () {
        toast.set(
  ...
  on_unfollow_btn_click: function on_unfollow_btn_click(btn, li_id, event) {
    var li = $(li_id);
    if (li.attr('type') == 'people') {
      var screen_name = li.attr('screen_name');
      toast.set(_('unfollow_at') + screen_name + ' ' + _('dots')).show();
      globals.twitterClient.destroy_friendships(screen_name, function () {
        toast.set(
      ...
      globals.twitterClient.destroy_list_subscriber(screen_name, slug, function () {
        toast.set(
  ...
  on_open_link_btn_click: function on_open_link_btn_click(btn, li_id, event) {
    var li = $(li_id);
    var link = li.attr('link');
    if (link.length > 0) window.open(link, '_blank');
  },

  on_open_people_btn_click: function on_open_people_btn_click(btn, li_id, event) {
    var li = $(li_id);
    var screen_name = (li.attr('screen_name') == '' || li.attr('screen_name') == undefined) ? li.attr('sender_screen_name') : li.attr('screen_name');
  ...
  on_thread_more_click: function on_thread_more_click(btn, event) {
  ...
  on_expander_click: function on_expander_click(btn, event) {
    var li = ui.Main.ctrl_btn_to_li(btn);
    var id = li.attr('retweet_id') == '' ? li.attr('tweet_id') : li.attr('retweet_id');
    var reply_id = li.attr('reply_id');
  ...
  load_thread_proc: function load_thread_proc(listview, tweet_id, on_finish, on_error) {
    var load_thread_proc_cb = function (prev_tweet_obj) {
      ...
      var reply_id = prev_tweet_obj.hasOwnProperty('in_reply_to_status_id_str') ? prev_tweet_obj.in_reply_to_status_id_str : prev_tweet_obj.in_reply_to_status_id;
    ...
    db.get_tweet(tweet_id, function (tx, rs) {
      if (rs.rows.length == 0) {
        globals.twitterClient.show_status(tweet_id, function (result) {
  ...
  preload_thread: function preload_thread(listview, tweet_obj) {
    ...
    reply_id = reply_id.toString();
    db.get_tweet(reply_id, function (tx, rs) {
      if (rs.rows.length != 0) {
  ...
  move_to_tweet: function move_to_tweet(pos) {
    var target = null;
    var current = null;
    var cur_view = null;
    ...
    if (current == null || current.length == 0) {
      ...
      if (!cur_view.hasOwnProperty('selected_item_id')) {
        cur_view.selected_item_id = '#' + cur_view._body.find('.card:first').attr('id');
      }
  ...
  move_by_offset: function move_by_offset(offset) {
    var current = null;
    var cur_view = null;
    if (ui.Main.selected_tweet_id != null) {
      current = $(ui.Main.selected_tweet_id);
  ...
  set_active_tweet_id: function set_active_tweet_id(id) {
    ui.Main.active_tweet_id = id;
  },

  openTweetMoreMenu: function openTweetMoreMenu(li, btn) {
    ...
    $('#tweet_more_menu').css({
      'left': (btn.offset().left - 135) + 'px',
      'top': (btn.offset().top - 42) + 'px'
    }).show();
    ui.Main.isTweetMoreMenuClosed = false;
  },

  closeTweetMoreMenu: function closeTweetMoreMenu() {
  ...
  ctrl_btn_to_li: function ctrl_btn_to_li(btn) {
    return $($(btn).parents('.card')[0]);
  },

  preview_image: function preview_image(url) {
