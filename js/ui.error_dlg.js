...
ui.ErrorDlg = {
  ...
  add_error: function add_error(title, summary, techinfo) {
    var n = new Date();
    var time = n.getFullYear() + '-' + n.getMonth() + '-' + n.getUTCDate() + ' ' + n.getHours() + ':' + n.getMinutes() + ':' + n.getSeconds()
    ui.ErrorDlg.container.prepend('<li class="error_item"><div class="error_item_header"><span class="error_item_time">' + time + '</span><span class="error_item_title">' + title + '</span></div><div class="error_item_body"><div class="error_item_summary">' + summary + '</div><pre class="error_item_techinfo">' + techinfo + '</pre></div></li>');
  },

  alert: function alert(title, summary, techinfo) {
