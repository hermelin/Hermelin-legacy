...
ui.StatusBox = {
  ...
  init: function init() {
    ...
    $('#tbox_status').keydown(function (event) {
    ...
    $('#tbox_dm_target').click(function (event) {
      return false;
    });
  ...
  update_status: function update_status(status_text) {
    if (status_text.length != 0) {
      ...
      globals.twitterClient.update_status(status_text, draft.reply_to_id, function (result) {
  ...
  update_status_len: function update_status_len() {
  ...
  post_message: function post_message(message_text) {
  ...
  post_message_cb: function post_message_cb(result) {
    ui.StatusBox.change_mode(ui.StatusBox.MODE_TWEET);
  ...
  post_image: function post_image(msg) {
    var params = {
      'message': msg
    };
  ...
  save_draft: function save_draft(draft) {
  ...
  append_status_text: function append_status_text(text) {
  ...
  insert_status_text: function insert_status_text(text, pos) {
    if (pos == null) {
    ...
    $('#tbox_status').val(
      $('#tbox_status').val().substr(0, pos) + text + $('#tbox_status').val().substring(pos));
  },
  ...
  set_status_text: function set_status_text(text) {
    $('#tbox_status').val(text);
    $('#tbox_status').removeClass('hint_style');
  },
  ...
  move_cursor: function move_cursor(pos) {
