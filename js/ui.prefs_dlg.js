...
ui.PrefsDlg = {
  ...
  init: function init() {
    ...
    $('#sel_prefs_sys_font, #range_prefs_font_size, #tbox_prefs_custom_font, #chk_use_custom_font').bind('click change keypress blur', function (event) {
    ...
    $('#range_prefs_font_size, #tbox_prefs_proxy_port').blur(function (event) {
      ui.FormChecker.test_int_value(this);
    });
    $('#range_prefs_font_size').change(function (event) {
      $('#range_prefs_font_size_st').text($(this).val() + 'pt');
      ui.PrefsDlg.update_font_preview();
    });
    $('#range_prefs_line_height').change(function (event) {
    ...
    $('#chk_prefs_use_same_sign_api_base').click(function (event) {
      $('#tbox_prefs_sign_api_base').attr('disabled', $(this).prop('checked'));
    });

    $('#chk_prefs_use_same_sign_oauth_base').click(function (event) {
    ...
    $('#sel_prefs_proxy_type').change(function (event) {
    ...
    $('#btn_prefs_ok').unbind().click(function (event) {
      ...
      if (err.count != 0) {
        ...
        widget.DialogManager.alert(
          'Oops, some mistakes in your information.', "<p>There are something wrong in what your changes.<br/>Please check errors in the options below:<br/> - " + err.error_values.join('<br/> - ') + '</p>');
      } else {
  ...
  switchPage: function switchPage(name) {
  ...
  save_settings: function save_settings() {
    // Globals
    conf.settings.use_verbose_mode = $('#chk_prefs_use_verbose_mode').prop('checked');
    conf.settings.close_to_exit = $('#chk_prefs_close_to_exit').prop('checked');
    ...
    // proxy
    if (util.is_native_platform()) {
      conf.settings.proxy_type = $('#sel_prefs_proxy_type').val();
      conf.settings.proxy_host = $('#tbox_prefs_proxy_host').val();
      conf.settings.proxy_port = parseInt($('#tbox_prefs_proxy_port').val());
      conf.settings.proxy_auth = $('#chk_prefs_proxy_auth').prop('checked');
      conf.settings.proxy_auth_name = $('#tbox_prefs_proxy_auth_name').val();
      conf.settings.proxy_auth_password = $('#tbox_prefs_proxy_auth_password').val();
  ...
  load_prefs: function load_prefs() {
    ...
    for (var i = 0, l = conf.vars.builtin_themes.length; i < l; i += 1) {
      ...
      }).text(theme_name).appendTo(theme_list);
    }
  ...
  save_prefs: function save_prefs() {
    var prefs = conf.get_current_profile().preferences;
    // Looks & Feels
    ...
    prefs.api_base = $('#tbox_prefs_api_base').attr('value');
    prefs.sign_api_base = $('#tbox_prefs_sign_api_base').attr('value');
    prefs.upload_api_base = $('#tbox_prefs_upload_api_base').attr('value');
    prefs.oauth_base = $('#tbox_prefs_oauth_base').attr('value');
    prefs.sign_oauth_base = $('#tbox_prefs_sign_oauth_base').attr('value');
  ...
  restore_defaults: function restore_defaults() {
  ...
  update_font_preview: function update_font_preview() {
    $('#prefs_font_preview')
      .css('font-family', $('#chk_use_custom_font').prop('checked') ? $('#tbox_prefs_custom_font').val() : conf.get_default_font_settings())
      .css({
      'font-size': $('#range_prefs_font_size').val() + 'pt',
      'line-height': $('#range_prefs_line_height').val()
    });
  }
