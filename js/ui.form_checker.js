...
ui.FormChecker = {
  ...
  check_config_error: function check_config_error(widgets) {
    var result = [];
    var count = 0
    $(widgets).each(function (idx, widget) {
      if ($(widget).data('error') == true) {
        count += 1;
        result.push($(widget).attr('title') + ': ' + $(widget).data('error_str'));
      }
    });
    return {
      'count': count,
  ...
  test_int_value: function test_int_value(widget) {
    var ret = false;
  ...
  test_int_range: function test_int_bound(widget, min, max) {
    var ret = false;
    var c_val = parseInt($(widget).val());
    if (isNaN(c_val)) {
      $(widget).addClass('test_fail');
      ...
      if (min < c_val && c_val < max) {
        ...
        $(widget).addClass('test_fail');
        $(widget).data('error', true);
        $(widget).data('error_str', ui.FormChecker.ERR_STR_INT_OUT_OF_RANGE + min + ' and ' + max);
      }
    }
    return ret;
  },

  test_text_len_limit: function test_text_len_limit(widget, max_len) {
    var ret = false;
  ...
  test_file_image: function test_file_image(file) {
    var ret = false;
    var filename = file;
  ...
  test_file_size_bound: function test_file_size_bound(file, bound) {
    var ret = false;
    if (file.size < bound) {
      ret = true;
    }
